...
from shared.role_mapping import ROLE_COLORS, normalize_role_webui
from shared.data_models import ReplayData
from shared.parsing_utils import discover_log_files, read_text_auto
...
def build_player(name: str, role: str | None, index: int) -> dict[str, Any]:
    role = normalize_role_webui(role)
    return {
        "name": name,
        "role": role,
        "alive": True,
...
def parse_game_log(file_path: Path) -> ReplayData:
    ...
    role_pattern = re.compile(r"【(?P<name>[^】]+)】.*?扮演(?P<desc>[^，。]+).*?身份是(?P<role>狼人|预言家|女巫|猎人|守护者|村民)|【(?P<old_name>[^】]+)】你在这场职场狼人杀中扮演(?P<old_role>[^，。]+)|【(?P<seat_name>\d+号)】.*?(?:扮演|身份是)(?P<seat_role>狼人|间谍|预言家|HR总监|女巫|CEO|猎人|法务总监|守护者|安保主管|村民)")
    ...
    vote_pattern = re.compile(r"投票结果：(?P<name>[^以]+)以(?P<count>\d+)票被(?:淘汰出局|全员投出，领大礼包走人)")
    inspect_pattern = re.compile(r"(?:查验结果|背调结果)：(?P<name>[^是]+)是(?P<camp>狼人|好人|间谍|清白员工)")
    attack_pattern = re.compile(r"今晚(?P<name>[^被]+)被(?:狼人击杀|间谍窃取了信息)")
    antidote_pattern = re.compile(r"(?:你使用解药救了|你签发了留人offer，)(?P<name>.+?)(?:被挽留)?$")
    poison_pattern = re.compile(r"(?:你使用毒药毒杀了|你签发了辞退信，)(?P<name>.+?)(?:被开除)?$")
    death_pattern = re.compile(r"昨夜，(?P<names>.+?)(?:不幸遇害|领了大礼包，正式离职)")
    ...
    for line in lines:
        ...
        if match:
            name = match.group("name") or match.group("old_name") or match.group("seat_name")
            role = normalize_role_webui(match.group("role") or match.group("old_role") or match.group("desc") or match.group("seat_role"))
...
class ReplayHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/api/replay":
            ...
            resolved = resolve_log_file(file_id)
            try:
                if resolved.suffix == ".jsonl":
                    from jsonl_parser import parse_game_log_jsonl
                    replay = parse_game_log_jsonl(resolved)
                else:
                    replay = parse_game_log(resolved)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="职场狼人杀日志回放 Web UI")
...
def main() -> None:
    ...
    _log.info(f"Replay UI running on http://{server.server_address[0]}:{server.server_address[1]}")
    _log.info("Press Ctrl+C to stop the server.")
    try:
        server.serve_forever()
    except KeyboardInterrupt:
        _log.info("Shutting down replay server...")
    finally:
        server.server_close()
